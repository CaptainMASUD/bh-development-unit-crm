...
function TemplateModal({
  ...
  // Auto focus the newly-added subtitle input
  useEffect(() => {
    if (!open) return
  ...
  const subHeading =
    mode === "edit"
      ...
      : "Create a reusable task title with structured subtitles."
...
/* ================= SKELETONS (match Customers page vibe) ================= */
...
export default function TaskTemplatePage() {
  ...
  const [sortMode, setSortMode] = useState("title") // title | newest

  // pagination
  const [nextCursor, setNextCursor] = useState(null)
  const [hasMore, setHasMore] = useState(false)

  // abort controller (optimization)
  ...
  const addSubtitleInput = () => setSubtitleValues((p) => [...(Array.isArray(p) ? p : [""]), ""])
  ...
  const createTemplate = async () => {
    ...
    try {
      const payload = {
        ...
        subtitles: normalizeSubs(subtitleValues),
      }
  ...
  // Skeleton behavior like Customers page:
  // - Show skeletons only when we're loading AND there is no data yet.
  // - When loadingMore, keep current list and append a couple skeleton rows.
  ...
  return (
    <div className="min-h-screen bg-gray-50/60 p-4 sm:p-6 lg:p-8">
      ...
      <TemplateModal
        ...
        onAddSubtitle={addSubtitleInput}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-5 sm:p-6 space-y-4">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
              <div className="flex gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-1">Fast search • Cursor pagination • Load more</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="relative">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search titles..." className={searchInputClass} />
              </div>
          ...
          <div className="divide-y divide-gray-100">
            {showRowSkeletons ? (
              ...
              <>
                      ...
                      <div className="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <button
                            ...
                            title={isOpen ? "Hide subtitles" : "Show subtitles"}
                          >
                            <FiChevronDown
                              className={[
                ...
                {/* Append skeletons while loading more (keeps UI feeling “alive”) */}
              ...
              <div className="p-10 text-center text-gray-500 text-sm">No titles found. Try clearing search.</div>
            )}
          </div>

          {/* Footer + Load more */}
          <div className="p-4 border-t border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <p className="text-xs text-gray-500">Loaded {sortedTemplates.length} • Page size {PAGE_SIZE}</p>
