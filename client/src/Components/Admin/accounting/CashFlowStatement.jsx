...
export default function CashFlowStatement() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3"><Stat title="Inflow" value={money(data.totals?.inflow)} /><Stat title="Outflow" value={money(data.totals?.outflow)} /><Stat title="Net Cash" value={money(data.totals?.net)} /></div>
      <div className={`${card} overflow-hidden`}><div className="overflow-x-auto"><table className="min-w-full divide-y divide-gray-100 text-left"><thead className="bg-gray-50 text-xs font-black uppercase tracking-[0.12em] text-gray-400"><tr>{["Source", "Inflow", "Outflow", "Net", "Rows"].map((h) => <th key={h} className="px-5 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-gray-100">{(data.items || []).map((r) => <tr key={r.sourceType}><td className="px-5 py-4 text-sm font-black">{pretty(r.sourceType)}</td><td className="px-5 py-4 text-sm font-black text-emerald-700">{money(r.inflow)}</td><td className="px-5 py-4 text-sm font-black text-rose-700">{money(r.outflow)}</td><td className="px-5 py-4 text-sm font-black">{money(r.net)}</td><td className="px-5 py-4 text-sm font-bold">{r.count}</td></tr>)}{!(data.items || []).length ? <tr><td colSpan={5} className="px-5 py-16 text-center text-sm font-bold text-gray-500">No cash flow rows.</td></tr> : null}</tbody></table></div></div>
    </div>
  )
}
