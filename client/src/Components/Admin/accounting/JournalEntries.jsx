...
export default function JournalEntries() {
  ...
  useEffect(() => { Promise.all([api("/accounting/accounts?limit=500"), api("/accounting/voucher-types"), api("/accounting/settings")]).then(([accountData, voucherData, settingsData]) => { setAccounts((accountData.accounts || []).filter((account) => !account.isGroup && account.isActive !== false)); setVoucherTypes(voucherData.voucherTypes || []); setSettings(settingsData.settings || {}) }).catch((error) => toast.error(error.message)) }, [])
