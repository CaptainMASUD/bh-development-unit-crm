...
async function api(path) {
  ...
  if (!response.ok) {
    throw new Error(
      data?.message ||
        data?.error ||
        "Unable to load serial tracking."
    )
  }
    ...
    >
      <div className="flex min-h-[50px] w-full flex-wrap items-center gap-2 rounded-2xl border border-gray-200 bg-gray-50/80 px-3 py-1.5 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          placeholder="Search reference, product or serial..."
          type="search"
...
function MobileSkeleton({ rows = 5 }) {
  return Array.from({ length: rows }).map((_, index) => (
    <article
      key={`serial-mobile-skeleton-${index}`}
      className="p-4"
    >
            ...
            {loading ? (
              ...
              rows.map((row, index) => (
                <tr
                  key={
                    row._id ||
                    `${row.trackingReference}-${row.serialNumber}-${index}`
                  }
      ...
      {!loading ? (
        <div className="flex flex-col gap-2 border-t border-gray-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs font-semibold text-gray-500">
            {rows.length}{" "}
            {rows.length === 1 ? "serial" : "serials"} shown
          </p>
...
export default function SerialNumbers() {
    ...
    async ({
      ...
      try {
        const data = await api(
          "/inventory/operations/tracking?type=serial"
        )
        ...
        if (showToast) {
          toast.success(
            "Serial tracking refreshed"
          )
        }
      } catch (e) {
        const message =
          e?.message ||
          "Failed to load serial tracking"
  ...
  const warehouseCount = useMemo(
    () =>
      new Set(
        rows
          .map((row) => relationKey(row.warehouse))
          .filter(Boolean)
      ).size,
    [rows]
  )
  ...
  return (
    ...
    >
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm font-medium text-gray-500">
                    Unit identity, location, availability, and quality state.
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-100 pt-4 lg:flex-row lg:items-center lg:justify-between">
              ...
              <span className="shrink-0 text-xs font-semibold text-gray-400">
                {loading
                  ? "Loading..."
                  : `${filtered.length} of ${rows.length} serials`}
              </span>
            </div>
          </div>
        </section>
      ...
      <ModalShell
        ...
        subtitle="Refine tracking by warehouse or quality state"
        icon={
          <Icon
            icon={FilterIcon}
            className="h-5 w-5"
