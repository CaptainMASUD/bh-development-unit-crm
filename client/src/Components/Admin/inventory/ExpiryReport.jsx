    ...
    >
      <div className="flex min-h-[40px] w-full flex-wrap items-center gap-1.5 rounded-2xl border border-gray-200 bg-[#f7f8fb] px-2.5 py-1 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        {qualityState !== "all" ? (
          <FilterChip
            label="State"
            value={pretty(qualityState)}
            onClear={() => setQualityState("all")}
          />
        ) : null}
        ...
        {expiryWindow !== "all" ? (
          <FilterChip
            label="Expiry"
            value={expiryLabels[expiryWindow] || pretty(expiryWindow)}
            onClear={() => setExpiryWindow("all")}
          />
        ) : null}

        <FocusPlaceholderInput
          className="h-8 min-w-[160px] basis-[200px] flex-1 border-0 bg-transparent px-1 py-0 text-sm font-medium text-gray-800 outline-none ring-0 shadow-none placeholder:text-gray-400 focus:border-0 focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0"
          style={{
            outline: "none",
            boxShadow: "none",
          }}
          ...
          placeholder="Search product, batch, serial..."
          type="search"
          aria-label="Search expiry tracking"
        />
...
function MobileSkeleton({
  ...
  return Array.from({
    ...
    <article
      key={`expiry-mobile-skeleton-${index}`}
      className="p-4"
    >
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-medium text-gray-500">
                    {row.trackingReference || "No reference"}
                  </p>
                </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-gray-100 px-3 py-2.5">
...
export default function ExpiryTracking() {
    ...
    async ({
      ...
      try {
        const data = await api(
          "/inventory/operations/tracking?type=expiry"
        )
  ...
  const qualityStates = useMemo(() => {
    return Array.from(
      new Set(
        rows
          .map((row) => clean(row.qualityState))
          .filter(Boolean)
      )
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (
        qualityState !== "all" &&
        clean(row.qualityState) !== qualityState
      ) {
        return false
      }
      ...
      if (
        expiryWindow !== "all" &&
        expiryMeta(row.expiryDate).key !== expiryWindow
      ) {
        return false
      }
  ...
  const expiryCounts = useMemo(() => {
    return rows.reduce(
      (result, row) => {
        const key = expiryMeta(row.expiryDate).key
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto w-full max-w-[1600px]">
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm font-medium text-gray-500">
                    Expiry visibility for proactive consumption and loss prevention.
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-100 pt-4 lg:flex-row lg:items-center lg:justify-between">
              <SearchFilters
                ...
                setExpiryWindow={setExpiryWindow}
          ...
          <div className="mb-4 flex items-start gap-3 rounded-2xl border border-rose-100 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-bold">
                Unable to load expiry tracking
              </p>
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className={cn(card, "p-4")}>
            ...
            <p className="mt-2 text-2xl font-extrabold tracking-tight text-rose-700">
              {number(expiryCounts.expired || 0, 0)}
            </p>
          </div>
          ...
          <div className={cn(card, "p-4")}>
            ...
            <p className="mt-2 text-2xl font-extrabold tracking-tight text-rose-600">
              {number(expiryCounts.within_7_days || 0, 0)}
            </p>
          </div>
          ...
          <div className={cn(card, "p-4")}>
            ...
            <p className="mt-2 text-2xl font-extrabold tracking-tight text-amber-700">
              {number(expiryCounts.within_30_days || 0, 0)}
            </p>
          </div>
        </div>
      ...
      <ModalShell
        ...
        title="Filter expiry tracking"
        subtitle="Refine by warehouse, quality state, or expiry window"
        icon={
          <Icon
            icon={FilterIcon}
            className="h-5 w-5"
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Quality State">
            <select
              ...
              value={qualityState}
              onChange={(event) =>
                setQualityState(event.target.value)
              }
            >
              <option value="all">
                All states
