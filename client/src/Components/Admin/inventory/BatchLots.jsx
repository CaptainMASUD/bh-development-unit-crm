    ...
    >
      <div className="flex min-h-[50px] w-full flex-wrap items-center gap-2 rounded-2xl border border-gray-200 bg-gray-50/80 px-3 py-1.5 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          placeholder="Search reference, product or batch..."
          type="search"
...
function MobileSkeleton({
  ...
  return Array.from({
    ...
    <article
      key={`batch-mobile-skeleton-${index}`}
      className="p-4"
    >
...
export default function BatchLots() {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `batch-tracking-${new Date().toISOString().slice(0, 10)}.csv`
      document.body.appendChild(a)
      a.click()
      a.remove()
      window.URL.revokeObjectURL(url)
      toast.success("Batch tracking CSV downloaded")
    } catch (err) {
    ...
    async ({
      ...
      try {
        const data = await api(
          "/inventory/operations/tracking?type=batch"
        )
        ...
        if (showToast) {
          toast.success(
            "Batch tracking refreshed"
          )
        }
      } catch (e) {
        const message =
          e?.message ||
          "Failed to load batch tracking"
  ...
  const totalUsable = useMemo(
    () =>
      rows.reduce(
        (sum, row) =>
          sum + Number(row.usableQuantity || 0),
        0
      ),
    [rows]
  )
  ...
  return (
    ...
    >
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm font-medium text-gray-500">
                    Batch-level quantities, positions, and quality traceability.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  title="Download batch tracking report as CSV"
                >
                  <Icon
                    icon={Download01Icon}
                    className={cn(
      ...
      <ModalShell
        ...
        subtitle="Refine batch tracking by warehouse or quality state"
        icon={
          <Icon
            icon={FilterIcon}
            className="h-5 w-5"
