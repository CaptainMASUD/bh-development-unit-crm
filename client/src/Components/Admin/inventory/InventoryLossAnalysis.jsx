    ...
    >
      <div className="flex min-h-[50px] w-full flex-wrap items-center gap-2 rounded-2xl border border-gray-200 bg-gray-50/80 px-3 py-1.5 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          type="search"
          aria-label="Search inventory losses"
        />
...
function MobileSkeleton({
  ...
  return Array.from({
    ...
    <article
      key={`loss-mobile-skeleton-${index}`}
      className="p-4"
    >
            ...
            {loading ? (
              ...
              rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="px-5 py-4">
                    <p className="max-w-[300px] truncate text-sm font-medium text-gray-700">
                      {row.reason || "-"}
                    </p>
                  </td>
                </tr>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs font-medium text-gray-500">
                    {row.lossReference || "No reference"} · {formatDate(row.lossDate)}
                  </p>
                </div>
...
export default function InventoryLossAnalysis() {
  ...
  const load = useCallback(
    async ({
      ...
      try {
        ...
        if (showToast) {
          toast.success(
            "Inventory loss analysis refreshed"
          )
        }
      } catch (e) {
        const message =
          e?.message ||
          "Failed to load inventory losses"
  ...
  const visibleLossValue = useMemo(
    () =>
      filtered.reduce(
        (sum, row) =>
          sum + Number(row.lossValue || 0),
  ...
  return (
    ...
    >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm font-medium text-gray-500">
                    Losses caused by damage, expiry, inspection, and adjustment.
                  </p>
                </div>
              </div>
          ...
          <div className="mb-4 flex items-start gap-3 rounded-2xl border border-rose-100 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-bold">
                Unable to load inventory losses
              </p>
          ...
          <div className={cn(card, "p-4")}>
            ...
            <div className="mt-2 flex items-end justify-between gap-3">
              <p className="text-2xl font-extrabold tracking-tight text-rose-600">
                {money(totalLossValue)}
              </p>

              {visibleLossValue !== totalLossValue ? (
                <p className="text-xs font-semibold text-gray-400">
                  {money(visibleLossValue)} visible
                </p>
              ) : null}
            </div>
          </div>
      ...
      <ModalShell
        ...
        title="Filter inventory losses"
        subtitle="Refine loss analysis by warehouse or loss type"
        icon={
          <Icon
            icon={FilterIcon}
            className="h-5 w-5"
