    ...
    >
      <div className="flex min-h-[40px] w-full flex-wrap items-center gap-1.5 rounded-2xl border border-gray-200 bg-[#f7f8fb] px-2.5 py-1 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          placeholder="Search reference, product, requester..."
          type="search"
          aria-label="Search stock requests"
        />
...
function MobileSkeleton({ rows = 5 }) {
  return Array.from({ length: rows }).map((_, index) => (
    <article
      key={`request-mobile-skeleton-${index}`}
      className="p-4"
    >
        ...
        <table className="w-full min-w-[1160px] text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              ...
              rows.map((row, index) => {
                const pending =
                  clean(row.status).toLowerCase() === "pending"

                const approved =
                  clean(row.status).toLowerCase() === "approved"
                      ...
                      <div className="flex justify-end gap-2">
                        {pending ? (
                          <>
                            <button
                              ...
                              disabled={actionKey === `${row._id}:approve`}
                              className={cn(
                                button,
                                successButton,
                                "px-3 py-2 text-xs"
                            ...
                            <button
                              ...
                              disabled={actionKey === `${row._id}:reject`}
                              className={cn(
                                button,
                                dangerButton,
                                "px-3 py-2 text-xs"
                        ...
                        {approved ? (
                          <button
                            ...
                            disabled={actionKey === `${row._id}:issue`}
                            className={cn(
                              button,
                              primaryButton,
                              "px-3 py-2 text-xs"
...
export default function StockRequests() {
  ...
  const action = async (
    ...
    if (!row?._id || actionKey) return

    const key = `${row._id}:${verb}`
    setActionKey(key)

    try {
      await api(
        `/requests/${row._id}/${verb}`,
        {
          method: "POST",
          body: JSON.stringify(
            verb === "reject"
              ? {
                  reason:
                    "Rejected during inventory review",
                }
              : {}
          ),
        }
  ...
  return (
    ...
    >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm font-medium text-gray-500">
                    Employee demand with independent approval, rejection, and issue controls.
                  </p>
                </div>
              </div>
      ...
      <ModalShell
        ...
        subtitle="Refine requests by status, department, or requester"
        icon={
          <Icon
            icon={FilterIcon}
            className="h-5 w-5"
