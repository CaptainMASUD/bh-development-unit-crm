...
const ENDPOINT = "/purchase/workflow/analyses"
...
function StatusBadge({ value }) {
  ...
  const styles = {
    ...
    po_created: "bg-emerald-50 text-emerald-700 ring-emerald-200",
  ...
  const dot =
    ["po_created", "completed", "active"].includes(status)
      ? "bg-emerald-500"
      : status === "rejected"
        ? "bg-rose-500"
        : ["pending", "submitted", "under_review", "awaiting_approval"].includes(status)
          ? "bg-amber-500"
    ...
    >
      <div className="flex min-h-[40px] w-full flex-wrap items-center gap-1.5 rounded-2xl border border-gray-200 bg-[#f7f8fb] px-2.5 py-1 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          placeholder="Search reference, request, product or supplier..."
          type="search"
          aria-label="Search purchase analysis"
        />
...
function FilterModal({
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title="Purchase-analysis filters"
      subtitle="Refine supplier-based workflow results"
...
function DetailsModal({ item, open, onClose }) {
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={item?.analysisReference || "Purchase-analysis details"}
      subtitle={
        item
          ? `${item.request?.requestReference || "Purchase request"} · ${
      ...
      {item ? (
        <div className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
              ...
              <p className="mt-2 text-lg font-black text-gray-950">
                {formatNumber(item.actualDemand)}
              </p>
            </div>
          ...
          <SectionCard
            ...
            description="Final purchase-analysis selection and quantity information."
          >
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ["Reference", item.analysisReference || "-"],
                ["Request", item.request?.requestReference || "-"],
...
function TableSkeleton({ rows = 8 }) {
  return Array.from({ length: rows }).map((_, index) => (
    <tr key={`purchase-analysis-skeleton-${index}`}>
...
function MobileSkeleton({ rows = 5 }) {
  return Array.from({ length: rows }).map((_, index) => (
    <article key={`purchase-analysis-mobile-skeleton-${index}`} className="p-4">
...
export default function PurchaseAnalysis() {
  ...
  const load = useCallback(async ({ showToast = false } = {}) => {
    ...
    try {
      ...
      const message = loadError?.message || "Failed to load purchase analysis"
      setError(message)
      toast.error(message)
    } finally {
      setLoading(false)
  ...
  const suppliers = useMemo(() => {
    ...
    rows.forEach((item) => {
      const source = item.selectedSupplier
      const key = String(
        normalizeId(source) ||
          source?.businessName ||
          source?.name ||
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((item) => {
      const supplierKey = String(
        normalizeId(item.selectedSupplier) ||
          item.selectedSupplier?.businessName ||
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm font-medium text-gray-500">
                Review demand, final quantities, supplier selection and value.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-rose-800">
                Could not load purchase analysis
              </p>
          ...
          <table className="w-full min-w-[1320px] text-left">
            <thead className="sticky top-0 z-20 bg-gray-50/95 backdrop-blur">
              <tr className="text-[11px] font-black uppercase tracking-[0.08em] text-gray-400">
                ...
                <th className="px-5 py-3.5">Final Quantity</th>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <p className="max-w-[240px] truncate text-sm font-semibold text-gray-900">
                        {relationLabel(item.selectedSupplier, "-")}
                      </p>
                    </td>
                ...
                <tr>
                  <td colSpan="9" className="px-5 py-14 text-center">
                    ...
                    <p className="mt-3 text-sm font-bold text-gray-900">
                      No purchase-analysis records found
                    </p>
                    <p className="mt-1 text-sm font-medium text-gray-500">
              ...
              >
                ...
                <div className="mt-4 rounded-xl border border-gray-100 px-3 py-2.5">
                  ...
                  <p className="mt-0.5 truncate text-xs font-medium text-gray-500">
                    {relationLabel(item.selectedSupplier, "No supplier")}
                  </p>
                </div>

                <div className="mt-3 grid grid-cols-2 gap-3 rounded-2xl bg-gray-50 p-3">
                  <div>
                    <p className="text-xs font-bold text-gray-400">Demand</p>
