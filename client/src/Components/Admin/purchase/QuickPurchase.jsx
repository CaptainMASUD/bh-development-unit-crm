...
const ENDPOINT = "/purchase/workflow/issues?purchaseType=quick_purchase"
...
function StatusBadge({
  ...
  const dot =
    ["purchased", "completed", "paid"].includes(status)
      ? "bg-emerald-500"
      : status === "rejected"
        ? "bg-rose-500"
    ...
    >
      <div className="flex min-h-[40px] w-full flex-wrap items-center gap-1.5 rounded-2xl border border-gray-200 bg-[#f7f8fb] px-2.5 py-1 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        <FocusPlaceholderInput
          ...
          type="search"
          aria-label="Search quick purchases"
        />
...
function FilterModal({
  ...
  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title="Quick-purchase filters"
      subtitle="Refine supplier and status results"
      icon={
        <Icon
          icon={FilterIcon}
          className="h-5 w-5"
    ...
    >
      {item ? (
        <div className="space-y-5">
          ...
          <SectionCard
            title="Quick purchase information"
            description="Quick-purchase issue data returned by the purchase workflow."
          >
...
function TableSkeleton({
  ...
  return Array.from({
    ...
    <tr
      key={`quick-purchase-skeleton-${index}`}
    >
...
function MobileSkeleton({
  ...
  return Array.from({
    ...
    <article
      key={`quick-purchase-mobile-skeleton-${index}`}
      className="p-4"
    >
  ...
  const load = useCallback(
    async ({
      ...
      try {
        ...
        if (showToast) {
          toast.success(
            "Quick purchases refreshed"
          )
        }
      } catch (loadError) {
        const message =
          loadError?.message ||
          "Failed to load quick purchases"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm font-medium text-gray-500">
                Review quick-purchase issues, supplier pricing and totals.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-rose-800">
                Could not load quick purchases
              </p>
                ...
                <tr>
                  ...
                  >
                    ...
                    <p className="mt-3 text-sm font-bold text-gray-900">
                      No quick purchases found
                    </p>

                    <p className="mt-1 text-sm font-medium text-gray-500">
                      Adjust the search or filters to find another quick purchase.
                    </p>
                  </td>
                </tr>
              )}
        ...
        <div className="flex flex-col gap-2 border-t border-gray-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {filtered.length ? (
            <span className="text-xs font-semibold text-gray-400">
              Purchase Management · Quick Purchase
            </span>
          ) : null}
        </div>
      </div>
