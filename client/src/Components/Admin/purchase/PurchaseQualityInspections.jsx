...
const ENDPOINT = "/purchase/workflow/quality-inspections"
...
function StatusBadge({ value }) {
  ...
  const styles = {
    ...
    inspected: "bg-indigo-50 text-indigo-700 ring-indigo-200",
  ...
  const dot =
    ["passed", "accepted", "completed"].includes(status)
      ? "bg-emerald-500"
    ...
    >
      <div className="flex min-h-[40px] w-full flex-wrap items-center gap-1.5 rounded-2xl border border-gray-200 bg-[#f7f8fb] px-2.5 py-1 transition focus-within:border-indigo-300 focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(99,102,241,0.10)]">
        ...
        {purchaseOrder !== "all" ? (
          <FilterChip
            label="Order"
            value={purchaseOrderName || "Selected order"}
            onClear={() => setPurchaseOrder("all")}
          />
        ) : null}
        ...
        <FocusPlaceholderInput
          ...
          placeholder="Search reference, order or product..."
          type="search"
          aria-label="Search purchase quality inspections"
        />
...
function FilterModal({
  ...
  return (
    <ModalShell
      ...
      subtitle="Refine purchase-order and status results"
...
function DetailsModal({ item, open, onClose }) {
  ...
  const acceptanceRate = purchased > 0 ? (accepted / purchased) * 100 : 0

  return (
    <ModalShell
      open={open}
      onClose={onClose}
      title={item?.inspectionReference || "Quality-inspection details"}
      subtitle={
        item
      ...
      {item ? (
        <div className="space-y-5">
          ...
          <SectionCard
            ...
            description="Quality result recorded against the purchased quantity."
          >
...
function MobileSkeleton({ rows = 5 }) {
  return Array.from({ length: rows }).map((_, index) => (
    <article key={`quality-mobile-skeleton-${index}`} className="p-4">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 rounded-2xl bg-gray-50 p-3">
...
export default function PurchaseQualityInspections() {
  const [rows, setRows] = useState([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState("")
  const [query, setQuery] = useState("")
  const [purchaseOrder, setPurchaseOrder] = useState("all")
  ...
  const load = useCallback(async ({ showToast = false, targetPage = 1 } = {}) => {
    ...
    try {
      ...
      const message =
        loadError?.message || "Failed to load quality inspections"
      setError(message)
      toast.error(message)
    } finally {
      setLoading(false)
  ...
  const purchaseOrders = useMemo(() => {
    ...
    rows.forEach((item) => {
      const source = item.purchaseOrder
      const key = String(
        normalizeId(source) || source?.orderNo || source?.name || ""
      )
      ...
      map.set(key, {
        key,
        name: source?.orderNo || relationLabel(source, "Purchase order"),
      })
    })
  ...
  const selectedPurchaseOrderName =
    purchaseOrder === "all"
      ? ""
      : purchaseOrders.find((item) => item.key === purchaseOrder)?.name ||
        "Selected order"

  const filterCount =
    (purchaseOrder !== "all" ? 1 : 0) +
    (status !== "all" ? 1 : 0)
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((item) => {
      const purchaseOrderKey = String(
        normalizeId(item.purchaseOrder) ||
  ...
  }, [rows, query, purchaseOrder, status])
      ...
      <section className={cn(card, "mb-5 p-4 sm:p-5")}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm font-medium text-gray-500">
                Review purchased, accepted and rejected quantities from quality checks.
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <SearchFilters
            ...
            purchaseOrderName={selectedPurchaseOrderName}
            status={status}
            setStatus={setStatus}
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                filtered.map((item, index) => (
                  <motion.tr
                    key={item._id || `${item.inspectionReference}-${index}`}
                ...
                <tr>
                  <td colSpan="8" className="px-5 py-14 text-center">
                    ...
                    <p className="mt-1 text-sm font-medium text-gray-500">
                      Adjust the search or filters to find another inspection.
                    </p>
                  </td>
                </tr>
              )}
            ...
            filtered.map((item, index) => (
              ...
              >
                ...
                <div className="mt-3 grid grid-cols-3 gap-3 rounded-2xl bg-gray-50 p-3">
                  <div>
                    <p className="text-xs font-bold text-gray-400">Purchased</p>
