 ...
 * Backend must enforce: employee can only get their assigned tasks
 * GET /notifications/deadlines/employee
 */
async function fetchEmployeeDeadlineNotifications({ windowDays = 365, includeOverdue = true, limit = 500 }) {
  ...
  const res = await fetch(`${API_BASE}/notifications/deadlines/employee?${qs.toString()}`, {
 ...
 * - Masud sees only Masud tasks
 * - Karim sees only Karim tasks
 * This is enforced by backend route:
 * GET /notifications/deadlines/employee
 */
export default function EmployeeNotificationModal({ open, onClose, isDarkMode, onOpenCustomer }) {
  ...
  useEffect(() => {
    ...
    // ✅ employee route (ONLY their tasks)
    fetchEmployeeDeadlineNotifications({ windowDays: 365, includeOverdue: true, limit: 500 })
      .then((arr) => {
        if (!alive) return
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-base font-bold truncate">My Deadlines</p>
