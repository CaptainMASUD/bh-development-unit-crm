...
function EmptyState({ title = "No items", subtitle = "Nothing to show right now." }) {
  return (
...
export default function DashboardContent() {
  ...
  const customerStats = useMemo(() => {
    ...
    const other = Math.max(0, total - inProgress - complete)
    ...
    return { total, inProgress, complete, other, assignedToMe }
  }, [customers, customersCount, currentUserId])
  ...
  const leadStats = useMemo(() => {
    ...
    const byStatus = {
      new: leads.filter((l) => safeLower(l?.status) === "new").length,
      contacted: leads.filter((l) => safeLower(l?.status) === "contacted").length,
      pending: leads.filter((l) => safeLower(l?.status) === "pending").length,
      confirmed: leads.filter((l) => safeLower(l?.status) === "confirmed").length,
      lost: leads.filter((l) => safeLower(l?.status) === "lost").length,
    }

    const assignedToMe = currentUserId
      ? leads.filter((l) => String(l?.assignedTo?._id || l?.assignedTo || "") === String(currentUserId)).length
      : 0

    return { total, ...byStatus, assignedToMe }
  }, [leads, leadsCount, currentUserId])
  ...
  const leadStatusChart = useMemo(() => {
    return [
      ...
      { name: "Confirmed", value: leadStats.confirmed },
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {me?.name ? (
                    <>
                      Welcome <span className="font-semibold text-gray-800">{me.name}</span> •{" "}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Pill
                ...
                label="Leads"
                value={isLoading && leads.length === 0 ? "—" : leadStats.total}
                tone="yellow"
              />
          ...
          <ChartCard title="Lead Status" subtitle="New vs Contacted vs Pending vs Confirmed vs Lost" icon={<FiLayers className="w-5 h-5" />}>
            <div className="h-64">
              {isLoading && leads.length === 0 ? (
                <div className="h-full flex items-center justify-center text-sm text-gray-500">Loading…</div>
              ) : leadStatusChart.length ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={leadStatusChart} margin={{ top: 8, right: 12, bottom: 0, left: 0 }}>
                    ...
                    <Bar dataKey="value" radius={[10, 10, 0, 0]}>
                      {leadStatusChart.map((_, i) => (
                        <Cell key={`l-${i}`} fill={CHART_COLORS[i % CHART_COLORS.length]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="xl:col-span-1 space-y-4">
          <div className={`${premiumCard} p-5`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Across customers</p>
              </div>
            </div>
          ...
          <div className={`${premiumCard} overflow-hidden`}>
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-white">
              <div className="flex items-center gap-3">
                <IconBadge icon={<FiTarget className="w-5 h-5" />} tone="yellow" />
                <div>
                  <h2 className="text-lg font-extrabold text-gray-900 tracking-tight">Recent Leads</h2>
                  <p className="text-sm text-gray-500">Most recently updated</p>
                </div>
              </div>
              <div className="text-sm text-gray-500">
                <span className="font-semibold text-gray-900">{Math.min(recentLeads.length, 8)}</span>
              </div>
            </div>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {l?.contact?.companyName ? (
                            <span className="font-semibold text-gray-700">{l.contact.companyName}</span>
                          ) : (
                            <span className="text-gray-400">No company</span>
                          )}
                          {l?.contact?.email ? <span className="text-gray-400"> • </span> : null}
                          {l?.contact?.email ? <span>{l.contact.email}</span> : null}
                        </p>
            ...
            <div className="p-4 space-y-3 bg-gray-50">
              <AnimatePresence>
                {isLoading && customers.length === 0 ? (
                  ...
                  <div className={`${premiumCard} bg-white`}>
                    <EmptyState title="No tasks in progress" subtitle="Everything looks good — new tasks will appear here." />
                  </div>
                )}
              </AnimatePresence>
            </div>
