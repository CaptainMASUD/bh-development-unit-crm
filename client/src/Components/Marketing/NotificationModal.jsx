...
async function fetchDeadlineNotifications({ windowDays = 365, includeOverdue = true, limit = 500 }) {
...
export default function NotificationModal({ open, onClose, isDarkMode, onOpenCustomer }) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState("")
  const [items, setItems] = useState([])
  const [query, setQuery] = useState("")
  const [activeTab, setActiveTab] = useState("upcoming") // ✅ default first tab
  ...
  useEffect(() => {
    ...
    // ✅ fetch bigger range so Upcoming can show more than 7 days
    fetchDeadlineNotifications({ windowDays: 365, includeOverdue: true, limit: 500 })
      .then((arr) => {
        if (!alive) return
  ...
  const enriched = useMemo(() => {
    ...
    return (Array.isArray(items) ? items : [])
      ...
      .filter((it) => {
        if (!q) return true
        const hay = `${it.title || ""} ${it.customerName || ""} ${it.companyName || ""}`.toLowerCase()
  ...
  const filtered = useMemo(() => {
    ...
    // ✅ Upcoming: latest due first (top)
    // ✅ Others: soonest due first
    if (activeTab === "upcoming") {
      arr.sort((a, b) => dueMs(b) - dueMs(a))
    } else {
      arr.sort((a, b) => dueMs(a) - dueMs(b))
    }

    return arr
  }, [enriched, activeTab])

  // ✅ NO ROUTE: open customer details same way AdminCustomersPage does
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-base font-bold truncate">Deadlines</p>
              <p className={cx("text-xs truncate", isDarkMode ? "text-gray-300" : "text-gray-600")}>
                Upcoming + overdue (filter tabs)
              </p>
            </div>
          </div>
        ...
        <div className="px-6 pt-5 pb-4">
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {[
                ["upcoming", "Upcoming", counts.upcoming], // ✅ first
                ["d3", "3 Days", counts.d3],
                ["d7", "7 Days", counts.d7],
                ...
                return (
                  ...
                  >
                    {label} <span className={cx("ml-1 opacity-80")}>{count}</span>
                  </button>
                )
              })}
            </div>
