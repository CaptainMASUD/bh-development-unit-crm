...
export const validateSalesPartyContext = async ({ branchId, customerId, dealId, salespersonId }) => {
  const [branch, customer, salesperson, deal] = await Promise.all([
    Branch.findOne({ _id: id(branchId, "branchId"), isActive: true }).lean(),
    Customer.findById(id(customerId, "customerId")).select("lifecycleStage status paymentTermsDays creditHold").lean(),
  ...
  if (!customer) throw new SalesError("The selected customer was not found in this company.", 404);
  ...
  if (deal && deal.customerId && String(deal.customerId) !== String(customerId)) {
  ...
  return { branch, customer, salesperson, deal };
};
