...
const resolveSystemAccount = async (code, tenantId = null, session = null) => {
  const settingField = {
  ...
  if (settingField) {
    let settings = null;
    if (tenantId) {
      const q = AccountingSettings.findOne({ tenantId, key: "company" }).select(settingField);
      if (session) q.session(session);
      settings = await q.lean();
    }
    if (!settings) {
      const q = AccountingSettings.findOne({ key: "company" }).select(settingField);
      if (session) q.session(session);
      settings = await q.lean();
    }
    const configured = settings?.[settingField];
    if (configured) {
      const accountFilter = tenantId ? { _id: configured, tenantId, isActive: true } : { _id: configured, isActive: true };
      const accountQ = Account.findOne(accountFilter);
      if (session) accountQ.session(session);
      let account = await accountQ.lean();
      if (!account && tenantId) {
        const fallbackQ = Account.findOne({ _id: configured, isActive: true });
        if (session) fallbackQ.session(session);
        account = await fallbackQ.lean();
      }
      if (account) return account._id;
    }
  }
  const accFilter = tenantId ? { code: String(code), tenantId, isActive: true } : { code: String(code), isActive: true };
  let account = await Account.findOne(accFilter).lean();
  if (!account && tenantId) {
...
const reconciliationResult = async (controlCode, subledgerTotal, label, tenantId = null) => {
  const accountId = await resolveSystemAccount(controlCode, tenantId);
...
export const getReceivables = async (req, res) => {
  try {
    ...
    const reconciliation = await reconciliationResult("1100", controlSubledgerTotal, "All open posted CRM and Sales invoices", req.tenantId || req.user?.company || null);
...
export const getPayables = async (req, res) => {
  try {
    ...
    const [settled, reconciliation] = await Promise.all([
      ...
      reconciliationResult("2000", controlSubledgerTotal, "All open posted supplier bills", req.tenantId || req.user?.company || null),
    ]);
...
const buildSettingsAccountOptions = async (tenantId = null) => {
  const accountFilter = tenantId
    ? {
        $and: [
          { isActive: true, isGroup: { $ne: true } },
  ...
  const [accounts, cashLinks, bankLinks] = await Promise.all([
    Account.find(accountFilter)
      .select("code name type subType controlType currency isActive isGroup tenantId")
...
export const getAccountingSettings = async (req, res) => {
  try {
    const tenantId = req.tenantId || req.user?.company || null;
    const filter = tenantId ? { tenantId, key: "company" } : { key: "company" };
    let settings = await populateSettings(AccountingSettings.findOne(filter));
    if (!settings && tenantId) {
      const globalDefault = await AccountingSettings.findOne({ key: "company", tenantId: null }).lean();
      if (globalDefault) {
        const { _id, createdAt, updatedAt, ...cloneable } = globalDefault;
        settings = await AccountingSettings.create({ ...cloneable, tenantId, key: "company" });
      } else {
        settings = await AccountingSettings.create({ tenantId, key: "company" });
      }
    } else if (!settings) {
      settings = await AccountingSettings.create({ key: "company" });
    }
    settings = await populateSettings(AccountingSettings.findById(settings._id));
    const accountOptions = await buildSettingsAccountOptions(tenantId);
...
export const updateAccountingSettings = async (req, res) => {
  try {
    const tenantId = req.tenantId || req.user?.company || null;
    const filter = tenantId ? { tenantId, key: "company" } : { key: "company" };
    const before = await AccountingSettings.findOne(filter).lean();
    const patch = {};
    for (const key of [
    ...
    if (ids.length) {
      ...
      const accountQuery = {
        ...
        isActive: true,
        isGroup: { $ne: true },
        ...(tenantId ? { $or: [{ tenantId }, { tenantId: null }, { tenantId: { $exists: false } }] } : {}),
      };
      const selectedAccounts = await Account.find(accountQuery).select("_id code name type subType controlType currency isActive isGroup tenantId").lean();
    ...
    const settings = await populateSettings(AccountingSettings.findOneAndUpdate(
      filter,
      { $set: patch, $setOnInsert: { ...filter, key: "company" } },
...
const vendorBillPostingLines = async (bill) => {
  ...
  if (Number(bill.taxAmount || 0) > 0) {
    lines.push({ account: await resolveSystemAccount("1200", bill.tenantId || null), debit: bill.taxAmount, credit: 0, description: `Input tax - ${bill.billNo}` });
  }
...
export const createVendorBill = async (req, res) => {
  try {
    ...
    const expenseAccount = match.matchStatus === "unlinked"
      ? (req.body.expenseAccount || await resolveSystemAccount("5000", tenantId))
