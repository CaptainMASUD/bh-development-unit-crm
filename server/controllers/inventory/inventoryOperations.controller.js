...
import { ConsumptionHistory, InventoryLoss, InventoryPreference, InventoryTracking, PendingInventory, StockInspection, StockIssue, StockRequest, WarehouseCheck } from "../../models/inventory/inventoryOperations.model.js";
...
const reference = (prefix, productCode = "") => `${prefix}-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}${productCode ? `-${productCode}` : ""}-${new mongoose.Types.ObjectId().toString().slice(-6).toUpperCase()}`;
...
async function productAndPosition(productId, warehouseId, locationId, session) {
  const [product, warehouse, location] = await Promise.all([
    Product.findById(productId).session(session), Warehouse.findById(warehouseId).session(session), WarehouseLocation.findById(locationId).session(session),
  ]);
  if (!product || product.status !== "active" || !product.trackInventory) throw Object.assign(new Error("Select an active inventory product."), { statusCode: 400 });
  if (!warehouse || warehouse.status !== "active") throw Object.assign(new Error("Select an active warehouse."), { statusCode: 400 });
  if (!location || location.status !== "active" || String(location.warehouse) !== String(warehouse._id)) throw Object.assign(new Error("Select an active bin belonging to the warehouse."), { statusCode: 400 });
  return { product, warehouse, location };
}

export const getPreferences = async (_req, res) => res.json(await InventoryPreference.findOneAndUpdate({ key: "default" }, { $setOnInsert: { key: "default" } }, { upsert: true, new: true }));
export const updatePreferences = async (req, res) => {
  try { const patch = { updatedBy: req.user._id }; if (["fifo", "lifo"].includes(req.body.stockIssueMethod)) patch.stockIssueMethod = req.body.stockIssueMethod; if (Number(req.body.nearExpiryAlertDays) > 0) patch.nearExpiryAlertDays = Number(req.body.nearExpiryAlertDays); if (typeof req.body.automaticProductCodes === "boolean") patch.automaticProductCodes = req.body.automaticProductCodes; const result = await InventoryPreference.findOneAndUpdate({ key: "default" }, { $set: patch }, { upsert: true, new: true, runValidators: true }); return res.json(result); } catch (e) { return fail(res, e, "Failed to update inventory preferences."); }
};

export const listPendingInventory = async (req, res) => { const { limit, skip } = page(req.query); const filter = req.query.status && req.query.status !== "all" ? { status: req.query.status } : {}; const [items, total] = await Promise.all([PendingInventory.find(filter).populate("product", "name sku imageUrl trackingType").sort({ arrivalDate: -1 }).skip(skip).limit(limit).lean(), PendingInventory.countDocuments(filter)]); res.json({ items, total, limit, skip }); };
export const createPendingInventory = async (req, res) => {
  try { const quantity = round(req.body.acceptedQuantity); if (quantity <= 0 || num(req.body.unitCost) < 0) return res.status(400).json({ message: "Accepted quantity and non-negative unit cost are required." }); const product = isId(req.body.product) ? await Product.findById(req.body.product) : null; if ((!product || product.status !== "active" || !product.trackInventory) && req.body.source !== "quick_purchase") return res.status(400).json({ message: "Normal inventory must use an active Product template." }); const item = await PendingInventory.create({ reference: clean(req.body.reference) || reference(req.body.source === "quick_purchase" ? "QP" : "PIN"), product: product?._id || null, quickProductName: clean(req.body.quickProductName), quickProductCode: clean(req.body.quickProductCode), acceptedQuantity: quantity, remainingQuantity: quantity, unitCost: num(req.body.unitCost), trackingType: product?.trackingType || req.body.trackingType || "none", source: req.body.source || "manual", purchaseReference: clean(req.body.purchaseReference), purchaseId: isId(req.body.purchaseId) ? req.body.purchaseId : null, arrivalDate: req.body.arrivalDate || new Date(), createdBy: req.user._id, updatedBy: req.user._id }); return res.status(201).json(item); } catch (e) { return fail(res, e, "Failed to create pending inventory."); }
};

export const assignPendingInventory = async (req, res) => {
  try { const result = await runMongoTransaction(async (session) => { const item = await PendingInventory.findById(req.params.id).session(session); if (!item || !["pending", "partially_assigned"].includes(item.status)) throw Object.assign(new Error("Pending inventory is not assignable."), { statusCode: 409 }); let product = item.product ? await Product.findById(item.product).session(session) : null; if (!product) { const code = item.quickProductCode || `QP${Date.now().toString().slice(-8)}`; [product] = await Product.create([{ name: item.quickProductName || `Quick Purchase ${code}`, sku: code, productType: "inventory", trackInventory: true, trackingType: item.trackingType, purchasePrice: item.unitCost, status: "active", createdBy: req.user._id, updatedBy: req.user._id }], sessionOpt(session)); item.product = product._id; } const quantity = round(req.body.quantity || item.remainingQuantity); if (quantity <= 0 || quantity > item.remainingQuantity) throw Object.assign(new Error("Assignment quantity exceeds the pending quantity."), { statusCode: 400 }); const { warehouse, location } = await productAndPosition(product._id, req.body.warehouse, req.body.location, session); const movement = await postMovement({ movementType: item.source === "quality_inspection" ? "purchase_receipt" : "opening_stock", referenceNo: item.reference, sourceType: "pending_inventory", sourceId: item._id, reason: "Pending inventory warehouse assignment", lines: [{ product: product._id, effect: "in", destinationWarehouse: warehouse._id, destinationLocation: location._id, quantity, requestedUnitCost: item.unitCost, lotNumber: clean(req.body.batchNumber), serialNumbers: req.body.serialNumbers || [], expiryDate: req.body.expiryDate || null }], userId: req.user._id, session });
      if (product.trackingType !== "none") { const serials = product.trackingType === "serial" ? req.body.serialNumbers || [] : [""]; if (product.trackingType === "serial" && serials.length !== quantity) throw Object.assign(new Error("Provide one serial number for every received unit."), { statusCode: 400 }); for (const serial of serials) await InventoryTracking.create([{ trackingReference: reference("TRK", product.sku), trackingType: product.trackingType, product: product._id, warehouse: warehouse._id, location: location._id, batchNumber: clean(req.body.batchNumber), serialNumber: clean(serial), arrivalDate: item.arrivalDate, expiryDate: req.body.expiryDate || null, originalQuantity: product.trackingType === "serial" ? 1 : quantity, remainingQuantity: product.trackingType === "serial" ? 1 : quantity, usableQuantity: product.trackingType === "serial" ? 1 : quantity, unitCost: item.unitCost, sourceType: item.source, sourceId: movement._id, createdBy: req.user._id, updatedBy: req.user._id }], sessionOpt(session)); }
...
export const issueStockRequest = async (req, res) => {
  try { const result = await runMongoTransaction(async (session) => { const requestDoc = await StockRequest.findById(req.params.id).session(session); if (!requestDoc || requestDoc.status !== "approved") throw Object.assign(new Error("Only approved requests can be issued."), { statusCode: 409 }); const stock = await ProductStock.findOne({ product: requestDoc.product, warehouse: req.body.warehouse, ...(req.body.location ? { location: req.body.location } : {}), status: "active", availableQuantity: { $gte: requestDoc.requestedQuantity } }).sort({ availableQuantity: -1 }).session(session); if (!stock) throw Object.assign(new Error("The selected warehouse does not contain sufficient available stock."), { statusCode: 409 }); const preference = await InventoryPreference.findOne({ key: "default" }).session(session); const method = preference?.stockIssueMethod || "fifo"; const product = await Product.findById(requestDoc.product).session(session); const allocations = []; if (product.trackingType !== "none") { let needed = requestDoc.requestedQuantity; const tracks = await InventoryTracking.find({ product: product._id, warehouse: stock.warehouse, location: stock.location, qualityState: "released", status: "active", usableQuantity: { $gt: 0 } }).sort({ arrivalDate: method === "fifo" ? 1 : -1, _id: method === "fifo" ? 1 : -1 }).session(session); for (const track of tracks) { if (needed <= 0) break; const used = Math.min(needed, track.usableQuantity); track.remainingQuantity = round(track.remainingQuantity - used); track.usableQuantity = round(track.usableQuantity - used); if (track.usableQuantity === 0) track.status = "depleted"; track.updatedBy = req.user._id; await track.save(sessionOpt(session)); allocations.push({ tracking: track._id, trackingReference: track.trackingReference, quantity: used }); needed = round(needed - used); } if (needed > 0) throw Object.assign(new Error("Eligible tracking records cannot fulfill the issue quantity."), { statusCode: 409 }); }
      const issueRef = reference("ISS", product.sku); const movement = await postMovement({ movementType: "production_issue", referenceNo: issueRef, sourceType: "stock_request", sourceId: requestDoc._id, reason: "Internal stock consumption", lines: [{ product: product._id, effect: "out", sourceWarehouse: stock.warehouse, sourceLocation: stock.location, quantity: requestDoc.requestedQuantity, requestedUnitCost: stock.averageCost }], userId: req.user._id, session }); const [issue] = await StockIssue.create([{ issueReference: issueRef, request: requestDoc._id, product: product._id, quantity: requestDoc.requestedQuantity, warehouse: stock.warehouse, location: stock.location, requester: requestDoc.requester, department: requestDoc.department, approver: requestDoc.approver, issuedBy: req.user._id, issueMethod: method, allocations, movement: movement._id }], sessionOpt(session)); await ConsumptionHistory.create([{ issue: issue._id, issueReference: issue.issueReference, issueDate: issue.issueDate, product: product._id, quantity: issue.quantity, employee: requestDoc.requester, department: requestDoc.department, warehouse: stock.warehouse, location: stock.location, allocations, approver: requestDoc.approver, issuedBy: req.user._id }], sessionOpt(session)); requestDoc.status = "issued"; requestDoc.issue = issue._id; requestDoc.updatedBy = req.user._id; await requestDoc.save(sessionOpt(session)); await writeAudit({ session, actorId: req.user._id, action: "issue", entityType: "StockRequest", entityId: requestDoc._id, after: requestDoc.toObject(), meta: getReqMeta(req) }); return issue; }); return res.status(201).json(result); } catch (e) { return fail(res, e, "Failed to issue stock."); }
};

export const listTracking = async (req, res) => { const { limit, skip } = page(req.query); const filter = {}; if (req.query.type && req.query.type !== "all") filter.trackingType = req.query.type === "expiry" ? { $in: ["expiry", "batch_expiry"] } : req.query.type; if (req.query.state && req.query.state !== "all") filter.qualityState = req.query.state; const [items, total] = await Promise.all([InventoryTracking.find(filter).populate("product", "name sku imageUrl").populate("warehouse location", "name code").sort({ arrivalDate: -1 }).skip(skip).limit(limit).lean(), InventoryTracking.countDocuments(filter)]); res.json({ items, total, limit, skip }); };
export const updateTrackingState = async (req, res) => { try { const state = req.body.qualityState; if (!["released", "quarantine", "removed"].includes(state)) return res.status(400).json({ message: "Invalid tracking quality state." }); const result = await runMongoTransaction(async (session) => { const track = await InventoryTracking.findById(req.params.id).session(session); if (!track) throw Object.assign(new Error("Tracking record not found."), { statusCode: 404 }); if (track.qualityState === state) return track; const quantity = track.usableQuantity; let movement = null; if (state === "removed" && quantity > 0) { movement = await postMovement({ movementType: "other", referenceNo: reference("REM"), sourceType: "tracking", sourceId: track._id, reason: clean(req.body.reason) || "Tracking removal", lines: [{ product: track.product, effect: track.qualityState === "quarantine" ? "out_quarantine" : "out", sourceWarehouse: track.warehouse, sourceLocation: track.location, quantity, requestedUnitCost: track.unitCost }], userId: req.user._id, session }); const [loss] = await InventoryLoss.create([{ lossReference: reference("LOS"), product: track.product, warehouse: track.warehouse, location: track.location, quantity, unitCost: track.unitCost, lossValue: quantity * track.unitCost, lossType: "tracking_removal", reason: clean(req.body.reason), sourceType: "tracking", sourceId: track._id, movement: movement._id, recordedBy: req.user._id }], sessionOpt(session)); void loss; track.remainingQuantity = 0; track.usableQuantity = 0; track.status = "removed"; } else if (quantity > 0) { const effect = state === "quarantine" ? "quarantine" : "release_quarantine"; movement = await postMovement({ movementType: state === "quarantine" ? "quarantine" : "release_quarantine", referenceNo: reference("TRS"), sourceType: "tracking", sourceId: track._id, reason: clean(req.body.reason), lines: [{ product: track.product, effect, sourceWarehouse: track.warehouse, sourceLocation: track.location, quantity }], userId: req.user._id, session }); } track.qualityState = state; track.updatedBy = req.user._id; await track.save(sessionOpt(session)); return track; }); return res.json(result); } catch (e) { return fail(res, e, "Failed to update tracking state."); } };

export const listConsumption = async (req, res) => { const { limit, skip } = page(req.query); const filter = {}; if (isId(req.query.product)) filter.product = req.query.product; if (isId(req.query.employee)) filter.employee = req.query.employee; if (isId(req.query.department)) filter.department = req.query.department; const [items, total] = await Promise.all([ConsumptionHistory.find(filter).populate("product", "name sku").populate("employee issuedBy approver", "name employeeId").populate("department", "name").populate("warehouse location", "name code").sort({ issueDate: -1 }).skip(skip).limit(limit).lean(), ConsumptionHistory.countDocuments(filter)]); res.json({ items, total, limit, skip }); };
export const listLosses = async (req, res) => { const { limit, skip } = page(req.query); const filter = req.query.lossType && req.query.lossType !== "all" ? { lossType: req.query.lossType } : {}; const [items, total, summary] = await Promise.all([InventoryLoss.find(filter).populate("product", "name sku").populate("warehouse location", "name code").sort({ lossDate: -1 }).skip(skip).limit(limit).lean(), InventoryLoss.countDocuments(filter), InventoryLoss.aggregate([{ $match: filter }, { $group: { _id: "$lossType", quantity: { $sum: "$quantity" }, value: { $sum: "$lossValue" }, count: { $sum: 1 } } }])]); res.json({ items, total, summary, limit, skip }); };
