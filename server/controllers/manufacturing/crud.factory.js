...
export const createCrudController = ({ Model, searchFields=[], populate=[] }) => ({
  ...
  create: async (req,res) => {
    assertTenant(req);
    const payload={...req.body,tenantId:req.tenantId,createdBy:req.user?._id||null,updatedBy:req.user?._id||null};
    delete payload._id;
    const type=DOCUMENT_NUMBER_TYPES.find((candidate)=>candidate.module==="manufacturing"&&candidate.model===Model.modelName);
    if(type) {
      const number=await assignDocumentNumber({tenantId:req.tenantId,typeKey:type.key,providedValue:payload[type.field],source:`manufacturing.${Model.modelName}.create`});
      payload[type.field]=number.value;
    }
    const item=await Model.create(payload);
    if(type) await finalizeDocumentNumberClaim({tenantId:req.tenantId,typeKey:type.key,value:item[type.field],recordId:item._id});
