...
export const createProductionPlan=async(req,res)=>{assertTenant(req);const item=await ProductionPlan.create({...req.body,tenantId:req.tenantId,planNumber:req.body.planNumber||await nextManufacturingNumber({tenantId:req.tenantId,key:"plan"}),createdBy:req.user?._id,updatedBy:req.user?._id});res.status(201).json({success:true,data:item});};
